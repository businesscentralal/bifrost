---
id: mcp-server
title: "BC MCP server (local setup)"
sidebar_label: "BC MCP server (local setup)"
sidebar_position: 25
description: "The origo-bc-mcp-server package runs a Model Context Protocol (MCP) server on your own machine that exposes Business Central metadata and data as MCP tools, so an AI assistant (Claude Code, VS Code Copilot, Claude Desktop, Cursor, etc.) can look up schema and call Bifröst…"
---
