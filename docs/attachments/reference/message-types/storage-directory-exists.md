...
Reports whether a directory exists in the configured storage connection.

## Metadata
- **Direction:** Outbound
- **Content-Type:** text/json
- **Invoke:** call the `call_message_type` tool with `type` = `Storage.Directory.Exists` and the parameters below as the `data` object.
...
| `path` | **Yes** | string | The directory path to check (relative to the connection base path). |
