...
description: "Request and response contract for the Sparisjodir.CreditCard.Transactions Bifröst message type."
---
...
|-------|------|----------|-------------|
| `cardId` | number | Yes | Numeric card identifier (from `Sparisjodir.CreditCard.Get`) |
...
- Get `cardId` from `Sparisjodir.CreditCard.Get`. The bank does not allow fetching all transactions in one call — always page (read `paging.totalPages` and iterate `pageNumber`).
...
- Upper bound = `statementDate`, capped at **yesterday** (Spar never returns transactions dated today; any future-dated rows are dropped and excluded from balances).
...
- `Statement Ending Balance` = the **negative** of the card's current outstanding balance (from `Sparisjodir.CreditCard.GetOne`) — e.g. a balance owed of 2,500 imports as −2,500.
...
**Requires:** `Spar Card Gate` permission set (the import calls the same CreditCardService operations as this message type).
