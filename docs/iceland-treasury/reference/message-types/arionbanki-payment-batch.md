...
sidebar_position: 31
description: "Request and response contract for the Arionbanki.Payment.Batch Bifröst message type."
---
...
Submits one or more payment batches to Arion banki **asynchronously** via the B2B `DoPayments` call. The response returns a `paymentsId`; use `Arionbanki.Payment.Result` or `Arionbanki.Payment.ResultBatch` to poll for per-line results.
...
- **Transfer lines (`kind = Transfer`)** — verify the destination account and its owner with `Arionbanki.Account.Verify` (`{bank, ledger, accountNumber, ownerPersonId}`) before adding the line. Only add the transfer when the response is `verified: true`. Confirm/normalize the source `outAccount` with `Arionbanki.Account.Get` or `Arionbanki.Account.GetByOwner`.
- **Payment-slip lines (`kind = PaymentSlip`)** — look up the claim with `Arionbanki.Bill.Get` (list outstanding bills) and `Arionbanki.Bill.GetDetails` (full slip incl. the payable amount) to confirm the amount and exact key before adding the line. Pay what the bank reports — do not compute the amount yourself.

Map a confirmed bill (from `Arionbanki.Bill.GetDetails`) onto a `PaymentSlip` line:
...
> `Arionbanki.Bill.GetDetails` requires BOTH `payorId` and `claimantId` (take them from the `Arionbanki.Bill.Get` row); see that type's help.
...
0. **Execution mode defaults to single-call production mode**: create exactly one `Arionbanki.Payment.Batch` call for one user request unless the user explicitly asks for testing, retries, or multiple batches.
...
2. Set `outAccount` to an Arion source account without separators.
...
7. Save `paymentsId` + `logEntryNo` from response and poll with `Arionbanki.Payment.ResultBatch`.
...
For a request like "create one batch with two lines", the agent must send exactly one `Arionbanki.Payment.Batch` call containing both lines.
...
## Async flow
1. Call `Arionbanki.Payment.Batch` with one or more batch objects.
2. Store the returned `paymentsId`.
3. Call `Arionbanki.Payment.Result` for one specific batch, or `Arionbanki.Payment.ResultBatch` for all batches under the `paymentsId` (with optional filter).
...
The response contains **only** the `paymentsId`. Full per-line results are available via `Arionbanki.Payment.ResultBatch`.
...
- Which BC Bank Account (`Bank Account No.`) matches the Arion source account (`outAccount`) — this is the credit leg for the journal.

### After receiving the response
1. **Write batch row**: `set_records` on table 10036185 "Arion Payment Batch":
   ...
   - `Bank Account No.` = the BC Bank Account that corresponds to the Arion source account
...
2. **Write line rows**: `set_records` on table 10036186 "Arion Payment Line" — one row per submitted line:
...
3. **Store** the `paymentsId` for later polling with `Arionbanki.Payment.Result` or `Arionbanki.Payment.ResultBatch`.
