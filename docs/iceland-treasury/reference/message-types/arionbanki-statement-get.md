...
sidebar_position: 33
description: "Request and response contract for the Arionbanki.Statement.Get Bifröst message type."
---
...
Retrieves a bank statement for an Arion account over a date span. The account header is returned once; transaction lines support skip/take paging. Transactions are returned **newest first**.

**Direction:** Outbound  
**Content-Type:** text/json

## Use when
- You need statement headers and transaction lines from an Arion bank account.
...
| Field | Use |
|---|---|
| `bankAccountNo` | BC bank account number. The connector resolves the Arion account from the BC bank account setup. |
| `normalizedAccount` | Direct Arion account number with separators (e.g. `0133-26-019566`). Use when querying without a BC bank account record. |
...
|---|---|---|
| `account` | string | Arion account number. |
