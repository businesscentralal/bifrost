...
sidebar_position: 5
description: "Request and response contract for the Arionbanki.Bill.Get Bifröst message type."
---
...
Retrieves all outstanding bills (kröfur) for the authenticated Arion banki user.
...
Vendor matching uses `claimantId` -> `Vendor."Registration Number"` and then open `Vendor Ledger Entry` matching on `amountDue`.
All open entries for the vendor are ranked and the best candidate wins, in this order: exact remaining amount + same due date, exact remaining amount, fee tolerance (max 999) + same due date, fee tolerance. Ties are broken by the smallest due-date distance, so recurring bills with identical amounts bind to the entry for the right period.
The due date compared is the bill's `finalDueDate` (eindagi), which is what BC stores as Due Date on the vendor ledger entry.
...
- Bill-level decision fields are explainable evidence:
  ...
  - `matchingDueDateMatched`: `true` when the matched entry carries the same due date as the bill's `finalDueDate`
...
- To fetch one bill's full details, call `Arionbanki.Bill.GetDetails` with that bill's `bank`, `ledger`, `number`, `dueDate`, **and both** `payorId` and `claimantId` (the bank requires both — see that type's notes).
- `dueDate` is returned as a timestamp (`YYYY-MM-DDT00:00:00`); pass only the date part (`YYYY-MM-DD`) to `Arionbanki.Bill.GetDetails`.
