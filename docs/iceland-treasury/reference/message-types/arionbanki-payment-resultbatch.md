...
sidebar_position: 32
description: "Request and response contract for the Arionbanki.Payment.ResultBatch Bifröst message type."
---
...
Queries the result of a previously submitted `Arionbanki.Payment.Batch` call for **one or all batches** under a `paymentsId`, with optional line-level filtering. Use this when you submitted multiple batches and need a combined status view.
...
## Response
Same `batches[].lines[]` shape as `Arionbanki.Payment.Result`, plus batch-level `dateOfPayment`, `dateOfForwardPayment`, `outAccount`, `outAccountOwnerId`, and per-line claim cost fields.
...
1. **For each batch in the response**: look up table 10036185 "Arion Payment Batch" by `Payments ID`.
2. **Update each batch row**: `set_records` on table 10036185:
   ...
   - If `Batch Status` changed since last poll: update table 10036186 "Arion Payment Line" `Batch Status` for all lines under the batch entry
...
4. **Create payment journal** when status is `Completed` or `CompletedWithErrors` and `Journal Created` = false:
   a. Read line rows: `get_records` on table 10036186 "Arion Payment Line" filtered by `Batch Entry No.` = the batch `Entry No.`
