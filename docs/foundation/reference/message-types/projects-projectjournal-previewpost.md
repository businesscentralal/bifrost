...
- **`Line Type` must be non-blank for headless previews.** BC's `Job Link Usage` (codeunit 1026) raises a CONFIRM dialog when `Line Type = " "`. Headless callers (MCP, scheduled task, web service) cannot answer it, so the preview errors out. Allowed non-blank values (BC `Job Line Type` enum names): `Budget`, `Billable`, `Both Budget and Billable`. Captions are localized — e.g. Billable appears as `Reikningshæft` on LCID 1039.
