...
Writes: `Sales Invoice Header."No. Printed"` is incremented (via Codeunit 315 `Sales Inv.-Printed`, from Report Selection usage `S.Invoice` — typically Report 1306). This call is not side-effect free despite returning a document. Out-of-box default; a replaced Report Selection report may omit the counter.
