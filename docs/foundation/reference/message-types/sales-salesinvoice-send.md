...
description: "Request and response contract for the Sales.SalesInvoice.Send Bifröst message type."
---
...
Triggers the BC standard "Send" action on a posted sales invoice. The action delivers the invoice to its recipient using whatever channels are configured on the resolved Document Sending Profile: email, printer, disk export, or an electronic document (PEPPOL/EHF/E-invoice) routed through a Document Exchange Service. This is the same action that runs when a user clicks **Send** on the posted sales invoice page, exposed as a Bifrost so external systems and agents can trigger it without a UI.

Use this message type when an upstream system (an MCP client, a workflow, an AI agent) needs to send a posted invoice to the customer right after posting, or to resend it later, without coding the channel logic. Channel selection (email vs print vs e-document) lives entirely in BC configuration via Document Sending Profiles; this message only decides *which* profile to use.

**Direction**: Inbound (action)  **Content-Type**: `text/json`

## What Happens When You Call This

1. **Resolve the invoice** - locate the `Sales Invoice Header` from the request (`invoiceId` GUID, `invoiceNo`, or `subject`).
2. **Resolve the profile** - pick a `Document Sending Profile` using the resolution chain below.
3. **Hand off to BC** - call `Sales Invoice Header.SendProfile(DocumentSendingProfile)`. From this point the behaviour is exactly the standard BC Send.
...
This message type does NOT return delivery confirmation. Delivery is asynchronous (especially for email and electronic documents). After the call returns `Success`, query the `Sales Invoice Header` to inspect what happened:
...
Use `Data.Records.Get` (table 112 `Sales Invoice Header`) to retrieve these fields after sending. The `Document Exchange Status` value is updated asynchronously by the DES connector job queue.

Sent-email history (subject, recipients, attachment, timestamp) is recorded by the BC base app in the `Sent Email` / `Email Outbox` tables and is reachable from the posted invoice via standard navigation.
...
2. **Customer profile** - `Customer."Document Sending Profile"` for the invoice's Sell-to Customer. This is the normal, per-customer routing.
...
2. Request JSON: `invoiceNo` / `invoiceId` (resolved into the subject via `ResolveDocumentSubject`).

## Request Parameters

| Field | Type | Required | Description |
|---|---|---|---|
| `invoiceNo` | string | One of these or `subject` | Sales Invoice Header `No.`. |
| `invoiceId` | GUID | One of these or `subject` | Sales Invoice Header `SystemId`. |
...
|---|---|---|
| `documentType` | string | Always `PostedSalesInvoice`. |
| `documentNo` | string | Sales Invoice Header `No.`. |
| `documentId` | GUID | Sales Invoice Header `SystemId`. |
| `customerNo` | Code[20] | Sell-to Customer No. on the invoice. |
| `customerName` | Text[100] | Sell-to Customer Name on the invoice. |
...
| Trigger | Error |
|---|---|
| Missing identifier | `Subject parameter is required. Provide the invoice number or SystemId.` |
| Invoice not found | `Sales Invoice {No} not found.` |
...
- `Sales.SalesCreditMemo.Send` - same behaviour for posted credit memos.
- `Data.Records.Get` (table 112) - read back `Sent as Email`, `Document Exchange Status`, `Document Exchange Identifier` to confirm delivery state.
