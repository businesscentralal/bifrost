...
This document describes the chat-related message types in the Bifrost Language Models extension.

## Overview

Bifrost Language Models contributes one message type to the Bifrost Foundation dispatcher: `LLM.Prompt.Complete`. It is a one-shot completion against the language model configured on a **Bifrost Language Model** record — a system prompt plus a user prompt go in, a text response comes back.
...
| Bifrost Language Models | 10035404 | Full access to the language models, the Bifrost Chat objects, the Copilot provider and the MCP tool server. Read-only on the Chat Gate. |
| Bifrost Language Models Read | 10035405 | Read-only access to the same objects. Language models can be inspected but not changed. |
