---
id: arionbanki-payment-batch
title: "Arionbanki.Payment.Batch"
sidebar_label: "Arionbanki.Payment.Batch"
sidebar_position: 31
description: "Beiðni- og svarsamningur fyrir Arionbanki.greiðsla.Batch Bifrastar-skilaboðagerð."
---
...
Sendir eina eða fleiri greiðsla batches til Arion banki **asynchronously** via the B2B `DoPayments` Kallaðu á. Svarið Skilar a `paymentsId`; Notaðu `Arionbanki.Payment.Result` eða `Arionbanki.Payment.ResultBatch` til poll fyrir per-line results.
...
- **Transfer lines (`kind = Transfer`)** — verify the destination reikningur og its owner með `Arionbanki.Account.Verify` (`{bank, ledger, accountNumber, ownerPersonId}`) áður en adding the line. Aðeins add the transfer Þegar Svarið er `verified: true`. Confirm/normalize the source `outAccount` með `Arionbanki.Account.Get` eða `Arionbanki.Account.GetByOwner`.
- **greiðsla-slip lines (`kind = PaymentSlip`)** — look up the claim með `Arionbanki.Bill.Get` (Listi outstanding bills) og `Arionbanki.Bill.GetDetails` (fulla slip incl. the payable amount) til confirm the amount og exact key áður en adding the line. Pay what the bank reports — do not compute the amount yourself.

Map a confirmed bill (frá `Arionbanki.Bill.GetDetails`) onto a `PaymentSlip` line:
...
| `personId` | `payorId` |
| `dueDate` | `dueDate` (the slip's gjalddagi; confirm against `Arionbanki.Bill.GetDetails`) |
| `amount` | `amountDue` / details `amount` |

> `Arionbanki.Bill.GetDetails` requires BOTH `payorId` og `claimantId` (take them frá the `Arionbanki.Bill.Get` row); see that Gerð's help.
...
```json
{
  "batches": [
    {
      "outAccount":           "0133260195661234",   // (required) Arion source account (no separators)
...
0. **Execution mode defaults til stakan-Kallaðu á production mode**: create exactly one `Arionbanki.Payment.Batch` Kallaðu á fyrir one user Beiðni unless the user explicitly asks fyrir testing, retries, eða multiple batches.
...
2. Set `outAccount` til an Arion source reikningur without separators.
...
7. Save `paymentsId` + `logEntryNo` frá Svar og poll með `Arionbanki.Payment.ResultBatch`.
...
fyrir a Beiðni like "create one batch með two lines", the agent verður að send exactly one `Arionbanki.Payment.Batch` Kallaðu á containing both lines.
...
## Async flow
1. Kallaðu á `Arionbanki.Payment.Batch` með eina eða fleiri batch objects.
2. Store the returned `paymentsId`.
3. Kallaðu á `Arionbanki.Payment.Result` fyrir one specific batch, eða `Arionbanki.Payment.ResultBatch` fyrir Allt batches under the `paymentsId` (með valfrjálst filter).
...
Svarið contains **Aðeins** the `paymentsId`. fulla per-line results eru available via `Arionbanki.Payment.ResultBatch`.
...
- Which BC bankareikningur (`Bank Account No.`) matches the Arion source reikningur (`outAccount`) — this er the credit leg fyrir the journal.

### eftir receiving Svarið
1. **Write batch row**: `set_records` on table 10036185 "Arion greiðsla Batch":
   ...
   - `Bank Account No.` = the BC bankareikningur that corresponds til the Arion source reikningur
...
2. **Write line rows**: `set_records` on table 10036186 "Arion greiðsla Line" — one row per submitted line:
...
3. **Store** the `paymentsId` fyrir later polling með `Arionbanki.Payment.Result` eða `Arionbanki.Payment.ResultBatch`.
