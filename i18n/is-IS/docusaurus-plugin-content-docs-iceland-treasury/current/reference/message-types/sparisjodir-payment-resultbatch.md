...
description: "Beiðni- og svarsamningur fyrir Sparisjodir.greiðsla.ResultBatch Bifrastar-skilaboðagerð."
---
...
Queries Niðurstaðan of a previously submitted `Sparisjodir.Payment.Batch` Kallaðu á fyrir **one eða Allt batches** under a `paymentsId`, með valfrjálst line-level filtering. Notaðu this Þegar you submitted multiple batches og need a combined status view.
...
## Svar
Same `batches[].lines[]` shape as `Sparisjodir.Payment.Result`, plus batch-level `dateOfPayment`, `dateOfForwardPayment`, `outAccount`, `outAccountOwnerId`, og per-line claim cost fields.
...
1. **fyrir each batch in Svarið**: look up table 10035960 "Spar greiðsla Batch" by `Payments ID`.
2. **Update each batch row**: `set_records` on table 10035960:
   ...
   - Ef `Batch Status` changed since last poll: update table 10035961 "Spar greiðsla Line" `Batch Status` fyrir Allt lines under the batch entry
...
4. **Create greiðsla journal** Þegar status er `Completed` eða `CompletedWithErrors` og `Journal Created` = false:
   a. Read line rows: `get_records` on table 10035961 "Spar greiðsla Line" filtered by `Batch Entry No.` = the batch `Entry No.`
