...
description: "Beiðni- og svarsamningur fyrir Landsbankinn.greiðsla.ResultBatch Bifrastar-skilaboðagerð."
---
...
Queries Niðurstaðan of a previously submitted `Landsbankinn.Payment.Batch` Kallaðu á fyrir **one eða Allt batches** under a `paymentsId`, með valfrjálst line-level filtering. Notaðu this Þegar you submitted multiple batches og need a combined status view.
...
`GetError` (singular) er accepted as a compatibility alias og treated as `GetErrors`.

## Svar
Same `batches[].lines[]` Svar shape as this Gerð's own result rows, plus batch-level `dateOfPayment`, `dateOfForwardPayment`, `outAccount`, `outAccountOwnerId`, og per-line claim cost fields.

## Leiðbeiningar fyrir gervigreind/umboð
Notaðu this message til reconcile one `paymentsId` at a time. Keep `filterStatus` explicit, Notaðu `GetAll` Aðeins Þegar the caller wants the fulla Svar, og treat `batchNo = 0` as the Allt-batches view. Þegar Niðurstaðan reaches a terminal status, Notaðu the confirmed data til update BC posting eða tracking færslur; do not invent bank amounts eða costs.
Persist `logEntryNo` frá every poll because it links til raw Beiðni/Svar evidence even Þegar internal tracking tables eru protected.
...
1. **fyrir each batch in Svarið**: look up table 10036064 "Lbi greiðsla Batch" by `Payments ID`.
2. **Update each batch row**: `set_records` on table 10036064:
   ...
   - Ef `Batch Status` changed since last poll: update table 10036065 "Lbi greiðsla Line" `Batch Status` fyrir Allt lines under the batch entry
...
4. **Create greiðsla journal** Þegar status er `Completed` eða `CompletedWithErrors` og `Journal Created` = false:
   a. Read line rows: `get_records` on table 10036065 "Lbi greiðsla Line" filtered by `Batch Entry No.` = the batch `Entry No.`
