...
description: "Beiðni- og svarsamningur fyrir Sparisjodir.reikningur.Sækja Bifrastar-skilaboðagerð."
---
...
Sækir Allt Sparisjóður accounts fyrir the authenticated service user.
...
- Feed those values í `Sparisjodir.Account.GetOne` (stakan reikningur + fulla details), `Sparisjodir.Account.Verify` (ownership check), eða `Sparisjodir.Account.GetByOwner` (Allt accounts fyrir one kennitala).
