...
sidebar_position: 17
description: "Beiðni- og svarsamningur fyrir Arionbanki.CreditCard.færslur Bifrastar-skilaboðagerð."
---
...
|-------|------|----------|-------------|
| `cardId` | number | Yes | Numeric card identifier (frá `Arionbanki.CreditCard.Get`) |
...
- Sækja `cardId` frá `Arionbanki.CreditCard.Get`. The bank does not allow fetching Allt færslur in one Kallaðu á — always page (read `paging.totalPages` og iterate `pageNumber`).
...
- Upper bound = `statementDate`, capped at **yesterday** (Arion never Skilar færslur dated today; any future-dated rows eru dropped og excluded frá balances).
...
- `Statement Ending Balance` = the **negative** of the card's current outstanding balance (frá `Arionbanki.CreditCard.GetOne`) — e.g. a balance owed of 2,500 imports as −2,500.
...
**Requires:** `Arion Card Gate` permission set (the import calls the same CreditCardService operations as this message Gerð).
