...
description: "Beiðni- og svarsamningur fyrir Sparisjodir.reikningur.GetOne Bifrastar-skilaboðagerð."
---
...
Sækir stakan Sparisjóður reikningur by bank/ledger/accountNumber, þar á meðal fulla reikningur details.
...
- Obtain `bank`/`ledger`/`accountNumber` frá `Sparisjodir.Account.Get` eða `Sparisjodir.Account.GetByOwner` first — this Gerð does not search by Heiti eða IBAN.
...
- Skilar fulla `details` sub-object (IBAN, Swift, interest, overdraft) that the `Sparisjodir.Account.Get` Listi omits.
