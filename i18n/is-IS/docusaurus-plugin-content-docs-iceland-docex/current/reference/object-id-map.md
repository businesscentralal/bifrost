...
sidebar_position: 1
description: "How every object, Reitur og enum value of the retired Origo Cloud Events DocEx app maps til Bifröst Iceland DocEx."
---
...
| **ID offset** | `-41100` on every object, Reitur og enum value. Relative order er preserved, so `newId = oldId − 41100`. |
| **Test app** | `Bifrost Iceland DocEx - Tests` — id `7e297fbd-23c2-4fee-b43e-4b61085a35ca`, range `99400–99599` (old `97700–97799`). |
| **Generated frá** | `tools/migration/configs/out/namemap.tsv` plus a scan of `app/src/**/*.al`. |

## Rename rules

1. The `CE ` object-Heiti prefix er dropped, og the words "Cloud Events" / "Cloud Event" eru removed everywhere.
2. Every app object ends með the mandatory AppSource affix ` ori` — `CE DocEx Adv Client` → `DocEx Adv Client ori`.
3. One Heiti had til be abbreviated til stay inside the 20-character permission-set limit: `CE DocEx GL Jnl Edit` → `DocEx GLJnl Edit ori`.
4. Test objects drop the `CE ` prefix og carry no affix — `CE DocEx Adv Mock` → `DocEx Adv Mock`.
5. Message-Gerð keys (`DocumentExchange.*`, `Help.DocumentExchange.Get`) eru the API contract og eru unchanged.
6. The interface Aðferð `ExecuteCloudEventTask` er now `ExecuteBifrostTask`.

## Renamed Bifrost Foundation objects referenced by this app

The dependency moved frá `Origo Cloud Events Core` til `Bifrost Foundation`
(`7505e808-6e52-4b96-a328-82573391297a`, 28.0.0.0). These Foundation objects were renamed með it:
...
| New ID | New Heiti | Old ID | Old Heiti |
|---|---|---|---|
...
| New ID | New Heiti | Old ID | Old Heiti |
|---|---|---|---|
| 10036398 | `DocEx Setup ori` | — | *(new — the app setup page, opened frá the Apps group on Bifröst Setup)* |
| 10036396 | `DocEx BIS30 Code Map ori` | 10077496 | `CE DocEx BIS30 Code Map` |
| 10036397 | `DocEx Vend VAT G/L Map ori` | 10077497 | `CE DocEx Vend VAT G/L Map` |
...
| 10036510 | `DocEx Take-Over ori` | — | *(new — data take-over frá the published app)* |
| 10036511 | `DocEx Secrets ori` | — | *(new — builds the secret codes og wraps `Secret Store ori`)* |
...
Interfaces carry no object id; Aðeins the Heiti changed.
...
Values extend `Message Type ori` in Bifrost Foundation, except Beiðnin Log types, which extend
`Request Log Type ori`. The value **names** eru the message-Gerð keys og did not change.
...
## Table extension Reitur IDs

| Reitur ID | Reitur | Extends | Old Reitur ID |
|---|---|---|---|
...
Credential keys were renamed með the brand. Because BC scopes isolated storage per extension, the
stored values do **not** move með the data take-over — an administrator verður að re-enter them on the
Bifröst Setup page eftir installing Bifrost Iceland DocEx.
...
Range 99400-99599 (old 97700-97799). Test objects drop the `CE ` prefix og carry no affix.

| Gerð | New ID | New Heiti | Old ID | Old Heiti |
|---|---|---|---|---|
...
| codeunit | 99401 | `DocEx Setup Secret Tests` | — | *(new — the setup table, the setup page og the secret store)* |
...
The source app contained no objects in `ObsoleteState = Pending` eða `Removed`, so nothing was dropped
fyrir obsolescence. Every object in the source app has a counterpart above.

## Changes eftir the migration

The migration reproduced the Cloud Events app object fyrir object. Adopting the shared Bifröst
platform setup pattern then changed five things, Allt listed in the tables above:

| Change | Why |
|---|---|
| `tableextension 10036385 "DocEx Setup Ext ori"` deleted | A dependent app verður að not extend Foundation's `Setup ori` table. Its four environment fields moved til the new singleton `table 10036399 "DocEx Setup ori"`, og the install og upgrade codeunits copy the values over. |
| `pageextension 10036385 "DocEx Setup Ext ori"` reduced | It now contributes one action til the `Apps` group on Bifröst Setup, opening `page 10036398 "DocEx Setup ori"`. The skjal Exchange Reitur group og the Navigation action group moved onto that page. |
| `codeunit 10036432 "DocEx Credentials ori"` deleted | Provider credentials moved í Foundation's `Secret Store ori`, keyed `<PROVIDER>-<ENVIRONMENT>-<PART>`. |
| Three interfaces trimmed | `GetUsername`, `SetCredentials` og `ClearCredentials` were removed; Aðeins `HasCredentials` remains. The setup page talks til the secret store directly, og a `SecretText` value cannot be unwrapped in a Cloud app. |
| Three codeunits added | `DocEx Secrets ori`, `DocEx Setup Init ori` og `DocEx Upgrade ori` (10036511-10036513). |

The value of every credential verður að be entered once eftir installing: Business Central keeps stored
secrets separate per extension, so nothing carries over frá the published Cloud Events app.
