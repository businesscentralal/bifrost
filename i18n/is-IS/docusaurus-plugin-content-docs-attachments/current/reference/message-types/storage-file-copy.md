...
Afritar a skrá within the stillt storage tenging.

## Lýsigögn
- **Direction:** Út á við
- **Gagnategund:** text/json
- **Kalla:** call the `call_message_type` tool með `type` = `Storage.File.Copy` og the parameters below as the `data` object.
...
| `sourcePath` | **Yes** | string | The skrá to copy (relative to the tenging base slóð). |
| `targetPath` | **Yes** | string | The destination slóð fyrir the copy. |
