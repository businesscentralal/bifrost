...
Lists the tags in a Clockify vinnusvæði.
...
```
`data` inniheldur an array of tag objects (each með `id`, `name`, `workspaceId`, `archived`).
...
No tracking action required — this er a read operation. Notaðu returned `id` gildi fyrir time-entry `tagIds`.
...
## Tengdar aðgerðir
- **Create tag:** `Clockify.Tag.Create`\- **Notaðu in tímafærslur:** Sendu `id` gildi in `tagIds` array of `Clockify.TimeEntry.Create`/`Update`\- **ID resolution:** Match on `name` to find the `id` needed fyrir write operations
