...
| **Schedule** | Opens the [Schedule Playbook](/help/nornir/schedule-playbook/) dialog and creates an orchestrator entry that runs the playbook on a recurring schedule. Disabled once the playbook is scheduled. |
| **Orchestrator Entry** | Opens the linked [orchestrator entry](/help/nornir/scheduled-entry-card/) so the schedule can be managed. Enabled only when the playbook is scheduled. |
| **Execution Log** | Opens the [Playbook Execution Log](/help/nornir/playbook-instances/) filtered to this playbook. |
