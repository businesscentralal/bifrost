...
**Clockify Workspaces** is a modal lookup that lists the Clockify workspaces the stored company API key can reach. It opens from the **Default Workspace ID** field on [Clockify Setup](/help/clockify/clockify-setup/); choosing a row writes both the workspace identifier and its name onto the setup card.
...
- **No key stored** — the lookup needs the company API key. Set it with **Set Company API Key** on [Clockify Setup](/help/clockify/clockify-setup/) first.
