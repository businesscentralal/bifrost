...
sidebar_label: "Clockify Setup"
sidebar_position: 2
---

The **Clockify Setup** card holds every setting of the Bifröst Clockify connector. It is reached from the **Clockify** action in the **Apps** group of the Bifröst **Setup** page, or by searching for *Clockify Setup*. There is exactly one setup record per company.

Nothing about Clockify is configured on Bifröst Foundation's own setup card — the connector keeps its settings here so it can be installed, configured and removed on its own.
...
| Default Workspace ID | The Clockify workspace used when a message request does not name one. Use the lookup to pick a workspace by name from the [workspace list](/help/clockify/clockify-workspace-lookup/). |
...
| Action | Description |
| --- | --- |
| Set Company API Key | Prompts for the Clockify API key in a [masked dialog](/help/clockify/clockify-set-secret-dialog/) and stores it in IsolatedStorage at company scope. |
...
| Integration Links | Opens the [Clockify Integration](/help/clockify/clockify-integration-list/) list of links between Business Central records and Clockify objects. |
| Registered Webhooks | Opens the [Clockify Webhooks](/help/clockify/clockify-webhooks/) list of webhooks registered from this company. |
