...
Run the action again to overwrite the stored key; there is no need to clear it first. To remove it altogether, use **Clear Company API Key** on [Timesheets Setup](/help/timesheets/timesheets-setup/). With no key stored, every message type that calls Clockify answers with a handled error saying the key is missing, rather than failing outright.
