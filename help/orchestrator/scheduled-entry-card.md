...
| **Client Credentials Code** | The [client credentials](/help/orchestrator/credentials-list/) used when this entry calls an external service. |
| **Earliest Start Date/Time** | The earliest date and time at which the job may run. |
| **Recurring Template Code** | A [recurring template](/help/orchestrator/recurring-template/) that supplies the schedule. When a template is selected, the fields in the Recurrence group become read-only and follow the template. |
...
| **Job Queue Entry** | Opens the [Job Queue Entry Card](/help/orchestrator/job-queue-entry-card/) for the underlying job queue entry. Enabled only while such an entry exists. |
