...
**Schedule Playbook** is the dialog that turns a playbook into a scheduled job. It is opened from the **Schedule** action on the [Bifrost Playbook](/help/orchestrator/playbook-card/) card, and on OK it creates an [orchestrator entry](/help/orchestrator/scheduled-entry-card/) that runs the playbook on the selected recurring schedule.

## Fields

| Field | Description |
| --- | --- |
| **Recurring Template Code** | The [recurring template](/help/orchestrator/recurring-templates/) that supplies the schedule. Mandatory – the dialog cannot be confirmed without it. |
...
-   If no template fits the pattern you need, create one first on [Job Queue Recurring Templates](/help/orchestrator/recurring-templates/).
