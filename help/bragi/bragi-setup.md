---
id: bragi-setup
title: "Bifröst Bragi Setup"
sidebar_label: "Bragi Setup"
sidebar_position: 5
---

**Bifröst Bragi Setup** is the single setup page of the chat module. It is opened from the **Apps** group on the Bifröst **Setup** page and shows, at a glance, whether the three things Bragi needs are in place: language models, the MCP tool server, and the provider API keys.
...
| Field | Description |
| --- | --- |
| **Language Models** | How many language models are set up in this company. Drill down to open the [language model list](/help/bragi/bifrost-lang-model-list/). |
...
Business Central keeps stored secrets separate per extension, so keys entered in an earlier version of the app cannot be carried over. Open a language model and use **Set Personal API Key** or **Set Shared API Key** on the [language model card](/help/bragi/bifrost-lang-model-card/) to enter each key once. Setting or clearing the shared key requires the `BIFROST ChatSvc ori` permission set.

## Actions

| Action | Description |
| --- | --- |
| **Language Models** | Opens the [language model list](/help/bragi/bifrost-lang-model-list/). |
| **API Keys** | Opens the Bifröst App Secrets list filtered to Bifrost Bragi, showing every key this module has registered and whether a value has been entered. The value itself is never shown. |
...
2.  Open **Bifröst Setup**, choose **Bifrost Bragi Setup** in the **Apps** group, and clear the HTTP client notification if it appears.
